...
int ORBmatcher::SearchByProjection(Frame &F,
    ...
    for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++) {
        ...
        const vector<size_t> vIndices = F.GetFeaturesInArea(pMP->mTrackProjX,
                pMP->mTrackProjY, r * F.mvScaleFactors[nPredictedLevel],
        ...
        for (vector<size_t>::const_iterator vit = vIndices.begin(), vend =
                vIndices.end(); vit != vend; vit++) {
...
bool ORBmatcher::CheckDistEpipolarLine(const cv::KeyPoint &kp1,
        const cv::KeyPoint &kp2, const cv::Mat &F12, const KeyFrame* pKF2) {
...
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw,
        const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched,
        int th) {
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2,
    ...
    for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++) {
        ...
        vector<size_t> vIndices2 = F2.GetFeaturesInArea(vbPrevMatched[i1].x,
                vbPrevMatched[i1].y, windowSize, level1, level1);
...
int ORBmatcher::SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2,
        cv::Mat F12, vector<pair<size_t, size_t> > &vMatchedPairs,
...
int ORBmatcher::SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2,
        vector<MapPoint*> &vpMatches12, const float &s12, const cv::Mat &R12,
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame,
    ...
    cv::cv2eigen(CurrentFrame.mTcw.rowRange(0, 3).colRange(0, 3), Rcw);

    //const cv::Mat tcw = CurrentFrame.mTcw.rowRange(0,3).col(3);
    Eigen::Matrix<float, 3, 1> tcw;
    cv::cv2eigen(CurrentFrame.mTcw.rowRange(0, 3).col(3), tcw);

    //const cv::Mat twc = -Rcw.t()*tcw;
    Eigen::Matrix<float, 3, 1> twc = -Rcw.transpose() * tcw;

    //const cv::Mat Rlw = LastFrame.mTcw.rowRange(0,3).colRange(0,3);
    Eigen::Matrix<float, 3, 3> Rlw;
    cv::cv2eigen(LastFrame.mTcw.rowRange(0, 3).colRange(0, 3), Rlw);

    //const cv::Mat tlw = LastFrame.mTcw.rowRange(0,3).col(3);
    Eigen::Matrix<float, 3, 1> tlw;
    cv::cv2eigen(LastFrame.mTcw.rowRange(0, 3).col(3), tlw);

    //const cv::Mat tlc = Rlw*twc+tlw;
    Eigen::Matrix<float, 3, 1> tlc = Rlw * twc + tlw;
